...
fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${client.user.username}&ownername=arman`)
    .then(res => res.json())
    .then(data => {
...
client.on("ready", () => {
    ...
    console.log("Join To Our Community https://discord.gg/3ayHsTk")
});

 require('http').createServer((req, res) => res.end('AutomodBot is alive! Join To Our Server')).listen(3000)
...
client.on("message", async message => {
  if (message.channel.name == "abotchat") {
    ...
    message.channel.startTyping();
    if (!message.content) return message.channel.send("Please say something.");
    scb.chat({message: message.content, name: client.user.username, owner:"Arman_HC", user: message.author.id, language:"auto"}).then(reply => {
